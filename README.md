...
🔹 خطوات تحسين تدريب النموذج
أثناء تدريب نموذج EfficientNet-B0، تم استخدام عدة تقنيات لتحسين الأداء وتقليل فرط التخصيص (Overfitting):

زيادة البيانات (Data Augmentation):
تطبيق تحويلات عشوائية (تدوير، قلب، قص، تغيير ألوان) لزيادة تنوع البيانات وتحسين تعميم النموذج.

المُحسّن AdamW:
استخدام AdamW بدلاً من خوارزميات SGD/Adam التقليدية لتحسين معالجة الـ Weight Decay وتحقيق تقارب أفضل.

تنعيم التسميات (Label Smoothing):
استخدام هذه التقنية لتقليل ثقة النموذج الزائدة وزيادة قدرته على التعميم.

التدريب بدقة مختلطة (Mixed Precision):
تفعيل torch.cuda.amp لتسريع التدريب وتقليل استهلاك ذاكرة الـ GPU دون التأثير على الدقة.

الإيقاف المبكر وحفظ أفضل نموذج:
حفظ أفضل نسخة من النموذج بناءً على دقة التحقق (Validation Accuracy) لتجنب فرط التخصيص.

جدولة معدل التعلم (Learning Rate Scheduling):
تعديل معدل التعلم بشكل ديناميكي لمساعدة النموذج على الوصول لأفضل نتيجة ممكنة.
